import datetime
import random
import json

from src.utils import create_file_field


PEOPLE = [
    "Микола Черновіл",
    "Оксана Петренко",
    "Іван Ковальчук",
    "Марія Данилішина",
    "Олег Кузьменко",
    "Тетяна Шевченко",
    "Павло Іваненко",
    "Катерина Козлова",
]
PAYMENT_DATA = {
    "organization_name": 'ГО "МУЗЕЙ КОЛИСКОВОЇ"',
    "edrpou": 45284215,
    "bank": 'АТ КБ "ПРИВАТБАНК"',
    "info": "Безповоротна фінансова допомога від прізвище, ім'я, по-батькові.",
    "iban": "UA353052990000026006035028980",
    "coffee_url": "https://www.buymeacoffee.com/kolyskova",
    "patreon_url": "https://www.patreon.com/KolyskovaMuseum",
    "qr_code_url": create_file_field("static/payment/Qr.jpg"),
}

FAKE_TEAM = [
    {
        "full_name": "Елеонора Хачатрян",
        "photo": create_file_field("static/our_team/team1.png"),
        "description": "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa",
    },
    {
        "full_name": "Олег Коробов",
        "photo": create_file_field("static/our_team/team2.png"),
        "description": "Nulla consequat massa quis enim. Donec pede justo, fringilla vel, aliquet nec, vulputate eget, arcu",
    },
    {
        "full_name": "Маргарита Скаженик",
        "photo": create_file_field("static/our_team/team3.png"),
        "description": "Aliquam lorem ante, dapibus in, viverra quis, feugiat a, tellus",
    },
]
FAKE_FOOTER = {
    "reporting": create_file_field("static/blank.pdf"),
    "privacy_policy": create_file_field("static/blank.pdf"),
    "rules_and_terms": create_file_field("static/blank.pdf"),
    "email": "1000i1songs@gmail.com",
    "facebook_url": "https://www.facebook.com/groups/1000Songs/",
    "youtube_url": "https://www.youtube.com/@Olegmaestro",
}
FAKE_ABOUT = {
    "content": """<p>Ласкаво просимо на сайт "1000 і 1 пісня" - унікальну онлайн платформу, присвячену збереженню та просуванню найкращих зразків української народної музики.<p><img src="https://1000and1songs.com/static/about/c2d7b6a9d0784f74b68fdbbf4963848a.jpeg"><img src="https://1000and1songs.com/static/about/885e9fcbb8b34cd3aa533aead8cc12a9.jpeg"><img src="https://1000and1songs.com/static/about/8af95a5c6b204223bc3ece07214f6fae.jpeg"></p>Наша історія починається з глибокого зацікавлення та поваги до багатої спадщини українського народу, яку ми намагаємося зберегти та подарувати майбутнім поколінням."1000 і 1 пісня" - це проект, заснований ентузіастами музики, які не лише прагнуть зберегти традиційні мелодії, але й дарувати їм нове життя через сучасні технології. Наша команда складається з талановитих музикантів, аранжувальників, звукорежисерів та етнографів, які мають спільну мету - зберегти та популяризувати українське музичне надбання.</p><p>Одним із ключових аспектів нашої діяльності є експедиції, які ми влаштовуємо по різних куточках України. Наш власник разом із командою вирушає у захоплюючі подорожі, де кожне село, кожна область є своєрідним музичним скарбницею. Ми прагнемо не лише записати ці пісні, але й збагатити їх новими аранжуваннями та інтерпретаціями, додаючи сучасний шар до традицій.</p><iframe class="ql-video" frameborder="0" allowfullscreen="true" src="https://www.youtube.com/embed/EDU2xd_bRvM?showinfo=0"></iframe><p><br></p><p>Кожна експедиція - це відкриття нових облич та зустріч з неповторною атмосферою кожного регіону. Ми вивчаємо та розуміємо місцеві особливості та унікальні ритми, що відзначають українську народну музику. Записуючи на багато доріжок, ми намагаємося передати всю глибину та розмаїття цього надзвичайного музичного спадку.</p><p>На нашому сайті ви зможете не лише послухати наші унікальні записи, але й дізнатися більше про історію кожної пісні, про те, як вона знаходиться у відділеній від часу світлині сучасності. Ми також регулярно ділимося інформацією про наші найновіші експедиції, події та проекти.</p><p>"1000 і 1 пісня" - це не просто колекція музики, це спроба зберегти душу та суть українського народу через його найбільш автентичний вираз - музику. Приєднуйтеся до нас у цьому захоплюючому подорожі через звуки та ритми України, допомагайте нам зберегти цей неповторний музичний спадок для майбутніх поколінь. "1000 і 1 пісня" - це ваш шлях до української музичної спадщини!</p>""",
    "title": "Про нас",
}
FAKE_NEWS_CATEGORY = [
    {"name": "Зустрічі"},
    {"name": "Лекції"},
    {"name": "Публікації"},
    {"name": "Майстер-класи"},
    {"name": "Концерти"},
    {"name": "Конференції"},
]
FAKE_NEWS = [
    {
        "title": "Майстеркласи для мішаного та чоловічого гурту у Торуні",
        "content": """<p>У Торуні, одному з найбільш привабливих міст Польщі, цієї неділі відбулися захоплюючі майстер-класи для мішаних та чоловічих гуртів, проведені відомим музикантом та харизматичним виконавцем Євгеном Єфремовим.</p><p><img src="https://1000and1songs.com/static/news/1.png"></p><p><img src="https://1000and1songs.com/static/news/2.png"></p><p>Майстер-класи пройшли у приміщенні Торунського етнографічного музею і зібрали учасників із різних куточків країни.</p><p><img src="https://1000and1songs.com/static/news/3.png"></p><p>А також гостей з-за кордону.</p>""",
        "short_description": "цієї неділі відбулися захоплюючі майстер-класи для мішаних та чоловічих гуртів",
        "authors": ["Поліщук Петро"],
        "editors": ["Даніель Санторо", "Навухо Доносор"],
        "photographers": ["Артас Менетіл", "Богдан Зара"],
        "preview_photo": create_file_field("static/news/1.png"),
        "created_at": datetime.datetime.now().date() - datetime.timedelta(days=1),
        "city_id": 1,
        "category_id": 1,
    },
    {
        "title": "Захоплюючі майстер-класи у  Кракові, в серці Польщі",
        "content": """<p>У Кракові, в історичному серці Польщі, відбулися захоплюючі майстер-класи, які були проведені видатним музикантом та харизматичним виконавцем Іваном Івановим.</p><p><img src="https://1000and1songs.com/static/news/1.png"></p><p>Ці заходи пройшли в приміщенні Краківського етнографічного музею і зібрали учасників із різних регіонів країни, а також гостей з-за кордону. Іван Іванов поділився своїм мистецтвом та досвідом з мішаними та чоловічими гуртами, надаючи непередавані враження та нові знання всім присутнім.</p><p><img src="https://1000and1songs.com/static/news/3.png"></p><p>Ця подія стала не лише музичним заходом, а й можливістю для обміну творчими ідеями серед талановитих артистів.</p>""",
        "short_description": "Майстер-класи, які були проведені видатним музикантом та харизматичним виконавцем Іваном Івановим",
        "authors": ["Поліщук Петро"],
        "editors": ["Даніель Санторо", "Навухо Доносор"],
        "photographers": ["Артас Менетіл", "Богдан Зара"],
        "preview_photo": create_file_field("static/news/2.png"),
        "created_at": datetime.datetime.now().date(),
        "city_id": 2,
        "category_id": 2,
    },
]
FAKE_FUNDS = [
    {"title": "Фонд ПНДЛ етномузикології НМАУ"},  # 1
    {"title": "Фонд ПНДЛ музичної етнології ЛНМА"},  # 2
    {"title": "Приватний фонд Людмили Іваннікової"},  # 3
    {"title": "Приватний фонд Маргарити Скаженик і Олега Коробова"},  # 4
    {"title": "Приватний фонд Віктора Ковальчука"},  # 5
    {"title": "Фонд ДНЦЗСТК"},  # 6
    {"title": "Приватний фонд Ірини Телюх"},  # 7
    {"title": "Приватний фонд Елеонори Хачатрян"},  # 8
    {"title": 'Фонд ГО "Моя провінція"'},  # 9
    {"title": "Приватний фонд Івана Синельнікова"},  # 10
    {"title": "Приватний фонд Вікторії Ярмоли"},  # 11
    {"title": "Приватний фонд Ірини Клименко"},  # 12
    {"title": "Приватний фонд Лариси Лукашенко"},  # 13
]
FAKE_COUNTRIES = [
    {"name": "Україна"},  # 1
    {"name": "Білорусь"},
    {"name": "Польща"},  # 3
    {"name": "Литва"},
    {"name": "Румунія"},  # 5
    {"name": "Молдова"},
    {"name": "Угорщина"},  # 7
    {"name": "Словаччина"},
    {"name": "Росія"},
]
FAKE_REGIONS = [
    {"country_id": 1, "name": "Київська область"},  # 1
    {"country_id": 1, "name": "Вінницька область"},
    {"country_id": 1, "name": "Волинська область"},  # 3
    {"country_id": 1, "name": "Дніпропетровська область"},
    {"country_id": 1, "name": "Донецька область"},  # 5
    {"country_id": 1, "name": "Житомирська область"},
    {"country_id": 1, "name": "Закарпатська область"},
    {"country_id": 1, "name": "Запорізька область"},  # 8
    {"country_id": 1, "name": "Івано-Франківська область"},
    {"country_id": 1, "name": "Кіровоградська область"},  # 10
    {"country_id": 1, "name": "Автономна республіка Крим"},
    {"country_id": 1, "name": "Луганська область"},
    {"country_id": 1, "name": "Львівська область"},  # 13
    {"country_id": 1, "name": "Миколаївська область"},
    {"country_id": 1, "name": "Одеська область"},  # 15
    {"country_id": 1, "name": "Полтавська область"},
    {"country_id": 1, "name": "Рівненська область"},
    {"country_id": 1, "name": "Сумська область"},  # 18
    {"country_id": 1, "name": "Тернопільська область"},
    {"country_id": 1, "name": "Харківська область"},  # 20
    {"country_id": 1, "name": "Херсонська область"},
    {"country_id": 1, "name": "Хмельницька область"},
    {"country_id": 1, "name": "Черкаська область"},  # 23
    {"country_id": 1, "name": "Чернівецька область"},
    {"country_id": 1, "name": "Чернігівська область"},  # 25
    {"country_id": 2, "name": "Берестейська область"},
    {"country_id": 2, "name": "Вітебська область"},
    {"country_id": 2, "name": "Гомельська область"},  # 28
    {"country_id": 2, "name": "Гродненська область"},
    {"country_id": 2, "name": "Могильовська область"},
    {"country_id": 2, "name": "Мінська область"},  # 31
    {"country_id": 3, "name": "Вармінсько-Мазурське воєводство"},
    {"country_id": 3, "name": "Великопольське воєводство"},
    {"country_id": 3, "name": "Західнопоморське воєводство"},  # 34
    {"country_id": 3, "name": "Куявсько-Поморське"},
    {"country_id": 3, "name": "Лодзинське воєводство"},
    {"country_id": 3, "name": "Люблінське воєводство"},  # 37
    {"country_id": 3, "name": "Любуське воєводство"},
    {"country_id": 3, "name": "Мазовецьке воєводство"},
    {"country_id": 3, "name": "Малопольське воєводство"},  # 40
    {"country_id": 3, "name": "Нижньосілезьке воєводство"},
    {"country_id": 3, "name": "Опольське воєводство"},
    {"country_id": 3, "name": "Підкарпатське воєводство"},  # 43
    {"country_id": 3, "name": "Підляське воєводство"},
    {"country_id": 3, "name": "Поморське воєводство"},
    {"country_id": 3, "name": "Свентокшиське воєводство"},  # 46
    {"country_id": 3, "name": "Сілезьке воєводство"},
    {"country_id": 9, "name": "Бєлгородська область"},
    {"country_id": 9, "name": "Брянська область"},  # 49
    {"country_id": 9, "name": "Волгоградська область"},
    {"country_id": 9, "name": "Воронезька область"},
    {"country_id": 9, "name": "Краснодарський край"},  # 52
    {"country_id": 9, "name": "Курська область"},
    {"country_id": 9, "name": "Ростовська область"},  # 54
    {"country_id": 9, "name": "Смоленська область"},
    {"country_id": 9, "name": "Псковська область"},  # 56
]

with open("result.json", "r") as file:
    city_data: dict = json.load(file)

FAKE_CITY = []
for key, values in city_data.items():
    FAKE_CITY.extend(values)

FAKE_GENRE = [
    {"genre_name": "Зима"},
    {"genre_name": "Колодій"},
    {"genre_name": "Масляна"},
    {"genre_name": "Ранцювання"},
    {"genre_name": "Весна"},
    {"genre_name": "Зелена неділя(Трійця)"},
    {"genre_name": "Купало"},
    {"genre_name": "Жнива"},
    {"genre_name": "Косовиця"},
    {"genre_name": "Осінь"},
    {"genre_name": "Оказія"},
    {"genre_name": "Весілля"},
    {"genre_name": "Колискові"},
    {"genre_name": "Родини/Хрестини"},
    {"genre_name": "Родини: Застілля"},
    {"genre_name": "Похорон"},
    {"genre_name": "Лірика"},
]

FAKE_SONG = [
    {
        "title": "Як вийду на гору - Copy",
        "song_text": "Як вийду на гору та й крикну й додому: - Та вари. мати, ой вечеряти, бо я йду й додому. - Та наварила, доню, небагацько - трошки. Та нема й тобі, ой доню моя, ні миски й ні ложки.",
        "song_description": "",
        "recording_date": datetime.date(2011, 8, 21),
        "performers": "Співачки з села Хитці",
        "ethnographic_district": "Полтавщина",
        "collectors": random.sample(PEOPLE, random.randint(1, len(PEOPLE))),
        "comment_map": "Test comment map",
        "video_url": "https://youtu.be/U01Pzqe9k_o",
        "map_photo": create_file_field("static/song/map.png"),
        "photo1": create_file_field("static/song/photo1.png"),
        "photo2": create_file_field("static/song/photo2.png"),
        "photo3": create_file_field("static/song/photo3.png"),
        "ethnographic_photo1": create_file_field("static/song/ethnographic1.jpg"),
        "ethnographic_photo2": create_file_field("static/song/ethnographic2.jpg"),
        "ethnographic_photo3": create_file_field("static/song/ethnographic3.jpg"),
        "stereo_audio": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio1": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio2": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio3": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio4": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio5": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio6": create_file_field("static/song/audio_example.mp3"),
        "city_id": 29,
        "fund_id": random.randint(1, 4),
    },
    {
        "title": "Ой якби ж я до Київа доріженьку знала",
        "song_text": "Ой якби ж я до Київа доріженьку знала, Я б своєму миленькому гостинця послала. Ниякого гостинчика – чотири горішки. Як немає чим приїхать, прийди, милий, пішки. І сам іде, коня веде до дівчини в гості. Провалився кінь вороний на кам’яним мості. Провалився й утопився ще й шапочка сплила. Молодая дівчинонька бистру річку брєла. Брєла річку, брєла другу понад берегами. Набачила рибаловця ж з чорними бровами. Риболовці славні хлопці вольніть мою волю – Закидайте тонкий невід по синьому морю. Тягнуть його ж до бережечка – з рота вода ллється. Молодая дівчинонька як орлиця б’ється. Ломить вона виломляє мазиного пальця – Ой нема вже миленького такого красавця.",
        "song_description": "",
        "recording_date": datetime.date(2011, 5, 9),
        "performers": "Анастасія Мартиненко, 1931 р.н.; Надія Погребиська, 1941 р.н.; Лідія Хачева, 1964 р.н.; Валентина Вангородська, 1940 р.н.",
        "ethnographic_district": "Середня Надніпрянщина",
        "collectors": random.sample(PEOPLE, random.randint(1, len(PEOPLE))),
        "comment_map": "Test comment map123",
        "map_photo": create_file_field("static/song/map.png"),
        "photo1": create_file_field("static/song/photo1.png"),
        "photo2": create_file_field("static/song/photo2.png"),
        "photo3": create_file_field("static/song/photo3.png"),
        "ethnographic_photo1": create_file_field("static/song/ethnographic1.jpg"),
        "ethnographic_photo2": create_file_field("static/song/ethnographic2.jpg"),
        "ethnographic_photo3": create_file_field("static/song/ethnographic3.jpg"),
        "video_url": "https://youtu.be/Zq5MxYrZO-E",
        "stereo_audio": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio1": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio2": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio3": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio4": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio5": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio6": create_file_field("static/song/audio_example.mp3"),
        "city_id": 20,
        "fund_id": random.randint(1, 4),
    },
    {
        "title": "Ой ти Галино",
        "song_text": "Зозуленька рябенькая Й усі луги облітала. А в одному не бувала. А в тім лузі теслі тешуть, Церкву ставляють з трьома йуглами, З трьома йокнами. В першім йокенці - ясне сонечко, В другім йокенці - ясен місяцю, В третім йокенці - ясні зірочки. Щедрий вечор, добрий вечор, Добрим людям на весь вечор! Примовки: Щедрівочка щедрувала, до віконця припадала, Що ти, тітко, наварила, що ти, тітко, напекла, принеси нам до вікна Не щипай, не ламай, а ціленького давай!",
        "song_description": "",
        "recording_date": datetime.date(2022, 1, 1),
        "performers": "Фольклорний ансамбль «Гуртоправці»",
        "ethnographic_district": "Середнє Полісся",
        "collectors": random.sample(PEOPLE, random.randint(1, len(PEOPLE))),
        "map_photo": create_file_field("static/song/map.png"),
        "video_url": "https://youtu.be/U01Pzqe9k_o",
        "photo1": create_file_field("static/song/photo1.png"),
        "photo2": create_file_field("static/song/photo2.png"),
        "photo3": create_file_field("static/song/photo3.png"),
        "ethnographic_photo1": create_file_field("static/song/ethnographic1.jpg"),
        "ethnographic_photo2": create_file_field("static/song/ethnographic2.jpg"),
        "ethnographic_photo3": create_file_field("static/song/ethnographic3.jpg"),
        "stereo_audio": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio1": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio2": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio3": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio4": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio5": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio6": create_file_field("static/song/audio_example.mp3"),
        "city_id": 33,
        "fund_id": random.randint(1, 4),
    },
    {
        "title": "Проведу я русалочки",
        "song_text": "",
        "song_description": "",
        "recording_date": datetime.date(2022, 1, 1),
        "performers": "Фольклорний ансамбль «Володар»",
        "ethnographic_district": "Середня Надніпрянщина",
        "collectors": random.sample(PEOPLE, random.randint(1, len(PEOPLE))),
        "map_photo": create_file_field("static/song/map.png"),
        "video_url": "https://youtu.be/U01Pzqe9k_o",
        "photo1": create_file_field("static/song/photo1.png"),
        "photo2": create_file_field("static/song/photo2.png"),
        "photo3": create_file_field("static/song/photo3.png"),
        "ethnographic_photo1": create_file_field("static/song/ethnographic1.jpg"),
        "ethnographic_photo2": create_file_field("static/song/ethnographic2.jpg"),
        "ethnographic_photo3": create_file_field("static/song/ethnographic3.jpg"),
        "stereo_audio": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio1": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio2": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio3": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio4": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio5": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio6": create_file_field("static/song/audio_example.mp3"),
        "city_id": 36,
        "fund_id": random.randint(1, 4),
    },
    {
        "title": "Косив козак сіно",
        "song_text": "",
        "song_description": "",
        "recording_date": datetime.date(2022, 1, 1),
        "performers": "Фольклорний ансамбль «Гуртоправці»",
        "ethnographic_district": "Середнє Полісся",
        "collectors": random.sample(PEOPLE, random.randint(1, len(PEOPLE))),
        "map_photo": create_file_field("static/song/map.png"),
        "video_url": "https://youtu.be/U01Pzqe9k_o",
        "photo1": create_file_field("static/song/photo1.png"),
        "photo2": create_file_field("static/song/photo2.png"),
        "photo3": create_file_field("static/song/photo3.png"),
        "ethnographic_photo1": create_file_field("static/song/ethnographic1.jpg"),
        "ethnographic_photo2": create_file_field("static/song/ethnographic2.jpg"),
        "ethnographic_photo3": create_file_field("static/song/ethnographic3.jpg"),
        "stereo_audio": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio1": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio2": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio3": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio4": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio5": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio6": create_file_field("static/song/audio_example.mp3"),
        "city_id": 8,
        "fund_id": random.randint(1, 4),
    },
    {
        "title": "Щедрик-ведрик, дайте",
        "song_text": "Щедрик-ведрик. Дайте вареник, Грудочку кашки, кольце ковбаски! А ще мало – дайте сала! А ще донесу – наверх ковбасу! А ще кишку – з’їм у затишку! Вигукують: Виносьте дохід добрий, Не ламайте та цілим давайте! Хоть коротенький, зате ситенький!",
        "song_description": "Щедрівка дитячого репертуару Фрагмент етнографічного документального фільму Єлєни Самойлової 'Святі вечори на Полтавщині'. Зйомки 13-14 січня 2009 року",
        "recording_date": datetime.date(2011, 5, 9),
        "performers": "Жителі села Крячківка",
        "ethnographic_district": "Середня Наддніпрянщина",
        "collectors": random.sample(PEOPLE, random.randint(1, len(PEOPLE))),
        "comment_map": "Щедрівки дитячого репертуару",
        "video_url": "https://youtu.be/U01Pzqe9k_o",
        "map_photo": create_file_field("static/song/map.png"),
        "photo1": create_file_field("static/song/photo1.png"),
        "photo2": create_file_field("static/song/photo2.png"),
        "photo3": create_file_field("static/song/photo3.png"),
        "ethnographic_photo1": create_file_field("static/song/ethnographic1.jpg"),
        "ethnographic_photo2": create_file_field("static/song/ethnographic2.jpg"),
        "ethnographic_photo3": create_file_field("static/song/ethnographic3.jpg"),
        "stereo_audio": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio1": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio2": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio3": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio4": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio5": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio6": create_file_field("static/song/audio_example.mp3"),
        "city_id": 7,
        "fund_id": random.randint(1, 4),
    },
    {
        "title": "А я знаю, що пан робіть",
        "song_text": "Щедрий вечор, добрий вечор. А я знаю, що пан робіть: Сидить собі в конці стола, А на йому шуба-люба, А в той шубі калиточка. В тей калитці сєм жучков, сєм жучков, Сєм гранічок, сєм тялічок. Дайте, дядьку, сєм палянічок!",
        "song_description": "Щедрівка господареві дитячого репертуару. Джерело: Щедрий вечор, добрий вечор. А я знаю, що пан робіть. «Рано-рано да зійду на гору». Серія «Традиційна музика Полісся».",
        "recording_date": datetime.date(2011, 5, 9),
        "performers": "Фольклорний гурт села Сарновичі",
        "ethnographic_district": "Середнє Полісся",
        "collectors": random.sample(PEOPLE, random.randint(1, len(PEOPLE))),
        "comment_map": "Поширення щедрівок з таким ритмічним типом показано на карті (див. легенду до карти)",
        "video_url": "https://youtu.be/U01Pzqe9k_o",
        "photo1": create_file_field("static/song/photo1.png"),
        "photo2": create_file_field("static/song/photo2.png"),
        "photo3": create_file_field("static/song/photo3.png"),
        "ethnographic_photo1": create_file_field("static/song/ethnographic1.jpg"),
        "ethnographic_photo2": create_file_field("static/song/ethnographic2.jpg"),
        "ethnographic_photo3": create_file_field("static/song/ethnographic3.jpg"),
        "stereo_audio": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio1": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio2": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio3": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio4": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio5": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio6": create_file_field("static/song/audio_example.mp3"),
        "city_id": 1,
        "fund_id": random.randint(1, 4),
    },
    {
        "title": "Зозуленька рябенькая",
        "song_text": "Зозуленька рябенькая Усі луги облітала, Й а в одному не бувала. Й а в тім лузі теслі тешуть Церкву ставляють З трьома углами, з трьома окнами. В першім й окенці – ясне сонечко, В другім й окенці – ясен місяцю, В третім й окенці – ясні зіроньки. Щодрий вечор, добрий вечор, Добрим людям на весь вечор. Примовки: Щедрівочка щедрувала, до віконця припадала, Що ти, тітко, наварила, що ти, тітко, напекла, принеси нам до вікна. Не щипай, не ламай, а ціленького давай!",
        "song_description": "Дитяча щедрівка з примовками. Поширення колядок і щедрівок з таким ритмом див. на карті далі",
        "recording_date": datetime.date(2015, 1, 1),
        "performers": "Виконавська реконструкція. Співає дитяча група фольклорного гурту Володар, м. Київ (кер. І. Клименко). СD Зберімося роде... Ч. 1. «Володар, одчиняй ворота!». Київ, 2003",
        "ethnographic_district": "Західне Полісся (Пінщина)",
        "collectors": random.sample(PEOPLE, random.randint(1, len(PEOPLE))),
        "comment_map": "Поширення колядок і щедрівок такого ритмічного типу показано на карті заливкою пурпурного кольору (див. також легенду до карти)",
        "video_url": "https://youtu.be/U01Pzqe9k_o",
        "photo1": create_file_field("static/song/photo1.png"),
        "photo2": create_file_field("static/song/photo2.png"),
        "photo3": create_file_field("static/song/photo3.png"),
        "ethnographic_photo1": create_file_field("static/song/ethnographic1.jpg"),
        "ethnographic_photo2": create_file_field("static/song/ethnographic2.jpg"),
        "ethnographic_photo3": create_file_field("static/song/ethnographic3.jpg"),
        "stereo_audio": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio1": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio2": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio3": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio4": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio5": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio6": create_file_field("static/song/audio_example.mp3"),
        "city_id": 2,
        "fund_id": random.randint(1, 4),
    },
    {
        "title": "Щедрик-ведрик, дайте-2",
        "song_text": "Щедрик-ведрик. Дайте вареник, Грудочку кашки, кольце ковбаски! А ще мало – дайте сала! А ще донесу – наверх ковбасу! А ще кишку – з’їм у затишку! Вигукують: Виносьте дохід добрий, Не ламайте та цілим давайте! Хоть коротенький, зате ситенький!",
        "song_description": "Щедрівка дитячого репертуару Фрагмент етнографічного документального фільму Єлєни Самойлової 'Святі вечори на Полтавщині'. Зйомки 13-14 січня 2009 року",
        "recording_date": datetime.date(2011, 5, 9),
        "performers": "Жителі села Крячківка",
        "ethnographic_district": "Середня Наддніпрянщина",
        "collectors": random.sample(PEOPLE, random.randint(1, len(PEOPLE))),
        "comment_map": "Щедрівки дитячого репертуару",
        "video_url": "https://youtu.be/U01Pzqe9k_o",
        "photo1": create_file_field("static/song/photo1.png"),
        "photo2": create_file_field("static/song/photo2.png"),
        "photo3": create_file_field("static/song/photo3.png"),
        "ethnographic_photo1": create_file_field("static/song/ethnographic1.jpg"),
        "ethnographic_photo2": create_file_field("static/song/ethnographic2.jpg"),
        "ethnographic_photo3": create_file_field("static/song/ethnographic3.jpg"),
        "stereo_audio": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio1": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio2": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio3": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio4": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio5": create_file_field("static/song/audio_example.mp3"),
        "multichannel_audio6": create_file_field("static/song/audio_example.mp3"),
        "city_id": 7,
        "fund_id": random.randint(1, 4),
    },
]
LOREM = """Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed vehicula libero nec justo convallis, nec sodales nunc pellentesque. Sed auctor efficitur sem id vestibulum. Mauris fringilla ullamcorper mauris eget pellentesque. Fusce ullamcorper elit vitae sapien tincidunt, vel molestie lacus tristique. Aliquam in ex ac risus auctor condimentum. Integer hendrerit tincidunt diam eget tempor. Integer sit amet condimentum dolor. Nullam viverra urna sit amet justo sagittis, non vehicula metus consectetur."""
SOURCES = """<html><body><ul><li><a href=\"https://kunsht.com.ua/articles/nastane-nash-chas\" target=\"_blank\">Куншт - настане наш час</a></li><li><a href=\"https://uk.wikipedia.org/wiki/%D0%94%D1%83%D0%BD%D0%B0%D1%8E,_%D0%94%D1%83%D0%BD%D0%B0%D1%8E,_%D1%87%D0%BE%D0%BC%D1%83_%D1%81%D0%BC%D1%83%D1%82%D0%B5%D0%BD_%D1%82%D0%B5%D1%87%D0%B5%D1%88%3F\" target=\"_blank\">Українські народні пісні</a></li><li><a href=\"https://uk.wikipedia.org/wiki/%D0%94%D1%83%D0%BD%D0%B0%D1%8E,_%D0%94%D1%83%D0%BD%D0%B0%D1%8E,_%D1%87%D0%BE%D0%BC%D1%83_%D1%81%D0%BC%D1%83%D1%82%D0%B5%D0%BD_%D1%82%D0%B5%D1%87%D0%B5%D1%88%3F\" target=\"_blank\">Дунаю, Дунаю, чому смутен течеш?</a></li></ul><p><br/></p></body></html>"""
FAKE_DESC_FOR_ES_SONG = """Ритуальні зимові обходи дворів зі спеціальними піснями тривають від Різдва до Водохреща. Мета обходів – побажати господарям добробуту і здоров’я на весь майбутній рік. За давніми (дохристиянськими) уявленнями учасники обходів виконують волю предків – опікунів роду. У більшості традицій на Різдво колядують (колядування могло тривати до Водохреща), ставлять Вертеп; напередодні Нового Року – щедрують (в зоні Карпат, в Галичині щедрування відсутні), водять Козу, Меланку, Коня тощо. На Новий рік посівають. Діти і молодь (хлопці, дівчата) колядують/щедрують попід хатою (під вікном або під дверима), зрідка могли заходити в сіни. Дорослих (одружених чоловіків і жінок), співаків церковної півчої – які співали колядки переважно християнської тематики – запрошують до хати. Ряджену «Козу» заводять до хати. Раніше господарі обдаровували колядників/щедрівників наїдками (ритуальне годування задобрення духів предків): пампушками, млинцями, коржиками, горіхами, насінням; пізніше – солодощами; у наш час – грошима."""
ES_MAIN_SONG_CATEGORY = [
    {
        "title": "Пісні зимового циклу",
        "description": FAKE_DESC_FOR_ES_SONG,
        "recommended_sources": SOURCES,
        "media": create_file_field("static/calendar_and_ritual_categories/1.jfif"),
    },
    {
        "title": "Пісні весняного циклу",
        "description": FAKE_DESC_FOR_ES_SONG,
        "recommended_sources": SOURCES,
        "media": create_file_field("static/calendar_and_ritual_categories/2.jfif"),
    },
    {
        "title": "Пісні літнього циклу",
        "description": FAKE_DESC_FOR_ES_SONG,
        "recommended_sources": SOURCES,
        "media": create_file_field("static/calendar_and_ritual_categories/3.jfif"),
    },
    {
        "title": "Весільні пісні",
        "description": FAKE_DESC_FOR_ES_SONG,
        "recommended_sources": SOURCES,
        "media": create_file_field("static/calendar_and_ritual_categories/4.jfif"),
    },
    {
        "title": "Родини і хрестини. Материнський і дитячий фольклор",
        "description": FAKE_DESC_FOR_ES_SONG,
        "recommended_sources": SOURCES,
        "media": create_file_field("static/calendar_and_ritual_categories/5.jfif"),
    },
    {
        "title": "Звичайні пісні",
        "description": FAKE_DESC_FOR_ES_SONG,
        "recommended_sources": SOURCES,
        "media": create_file_field("static/calendar_and_ritual_categories/6.jfif"),
    },
    {
        "title": "Музичний епос",
        "description": FAKE_DESC_FOR_ES_SONG,
        "recommended_sources": SOURCES,
        "media": create_file_field("static/calendar_and_ritual_categories/7.jfif"),
    },
    {
        "title": "Інструментальна музика. Народні танці",
        "description": FAKE_DESC_FOR_ES_SONG,
        "recommended_sources": SOURCES,
        "media": create_file_field("static/calendar_and_ritual_categories/8.jfif"),
    },
    {
        "title": "Традиційний похорон: музичне оформлення",
        "description": FAKE_DESC_FOR_ES_SONG,
        "recommended_sources": SOURCES,
        "media": create_file_field("static/calendar_and_ritual_categories/9.jpg"),
    },
]
FAKE_SUB_CATEGORY = [
    {
        "title": "Ранньотрадиційний репертуар",
        "main_category_id": 1,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Колядки пізнього походження (християнські)",
        "main_category_id": 1,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Рання весна. Перехід зима/весна",
        "main_category_id": 2,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Період весняного рівнодення. Великдень",
        "main_category_id": 2,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Зелені свята",
        "main_category_id": 2,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Приурочені ліричні пісні ранньої стилістики",
        "main_category_id": 3,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Купальсько-петрівські пісні",
        "main_category_id": 3,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Жнивні пісні",
        "main_category_id": 3,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Передвесільні обряди і пісні",
        "main_category_id": 4,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Весілля. Недільні обряди і пісні",
        "main_category_id": 4,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Післявесільні звичаї і пісні",
        "main_category_id": 4,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Музичне оформлення народин і хрестин",
        "main_category_id": 5,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Музичні жанри «материнського фольклору»",
        "main_category_id": 5,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Пісні дитячого репертуару",
        "main_category_id": 5,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Ліричні пісні (за музичними ознаками)",
        "main_category_id": 6,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Ліричні пісні (за тематикою)",
        "main_category_id": 6,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Коломийки",
        "main_category_id": 6,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Приспівки",
        "main_category_id": 6,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Пісенна творчість пізнішого часу",
        "main_category_id": 6,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Билини",
        "main_category_id": 7,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Думи",
        "main_category_id": 7,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Традиційний лірницький репертуар",
        "main_category_id": 7,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Українські традиційні музичні інструменти",
        "main_category_id": 8,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Інструментальні капели. «Троїста музика»",
        "main_category_id": 8,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Танцювальна музична культура українців",
        "main_category_id": 8,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Голосіння",
        "main_category_id": 9,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
    {
        "title": "Поминальні псальми",
        "main_category_id": 9,
        "media": create_file_field("static/song_subcategories/subimg.png"),
    },
]
FAKE_GENRE_ES = [
    {
        "main_category_id": 1,
        "sub_category_id": 1,
        "title": "Дитячі колядки та щедрівки",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 1,
        "sub_category_id": 1,
        "title": "Величальні колядки та щедрівки",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 1,
        "sub_category_id": 1,
        "title": "Пісні до новорічних карнавальних процесів",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 1,
        "sub_category_id": 1,
        "title": "Засівання",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 1,
        "sub_category_id": 2,
        "title": "Різдвяні канти",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 1,
        "sub_category_id": 2,
        "title": "Різдвяні псальми",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 1,
        "sub_category_id": 2,
        "title": "Пародійні колядки",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 1,
        "sub_category_id": 2,
        "title": "Вертеп",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 3,
        "title": "Проводи Зими",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 3,
        "title": "Колодка",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 3,
        "title": "Масляні пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 3,
        "title": "Зустріч весни",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 3,
        "title": "Креснянка",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 3,
        "title": "Каша",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 4,
        "title": "Великодні ранцівки",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 4,
        "title": "Весняні танки та ігри",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 5,
        "title": "Похорон Стріли",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 5,
        "title": "Водіння Куста",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 2,
        "sub_category_id": 5,
        "title": "Русальні пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 3,
        "sub_category_id": 7,
        "title": "Купальсько-петрівські пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 3,
        "sub_category_id": 8,
        "title": "Жнивні пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 3,
        "sub_category_id": 6,
        "title": "Ягідні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 3,
        "sub_category_id": 6,
        "title": "Спасовки",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 3,
        "sub_category_id": 6,
        "title": "Строкові",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 3,
        "sub_category_id": 6,
        "title": "Косарські",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 3,
        "sub_category_id": 6,
        "title": "Літо",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 3,
        "sub_category_id": 6,
        "title": "До полоття",
        "description": LOREM,
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 4,
        "sub_category_id": 9,
        "title": "Передвесільні обряди і пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 4,
        "sub_category_id": 10,
        "title": "Весілля. Недільні обряди і пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 4,
        "sub_category_id": 11,
        "title": "Післявесільні звичаї і пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 5,
        "sub_category_id": 12,
        "title": "Народини та хрестини",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 5,
        "sub_category_id": 13,
        "title": "Колискові пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 5,
        "sub_category_id": 13,
        "title": "Забавлянки",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 5,
        "sub_category_id": 13,
        "title": "Казки зі співом",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 5,
        "sub_category_id": 14,
        "title": "Колядки, щедрівки",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 5,
        "sub_category_id": 14,
        "title": "Веснянки",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 5,
        "sub_category_id": 14,
        "title": "Оказіональні приспівки до дощу, сонця, птахів",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 15,
        "title": "Ранні ліричні пісні ",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 15,
        "title": "Багатоголосні пісні з сольним виводом",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 16,
        "title": "Про кохання",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 16,
        "title": "Про родинні відносини",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 16,
        "title": "Балади",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 16,
        "title": "Чумацькі пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 16,
        "title": "Бурлацькі пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 16,
        "title": "Наймитські пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 16,
        "title": "Козацькі пісні, історичні пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 16,
        "title": "Рекрутські пісні, солдатські пісні, жовнірські пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 16,
        "title": "Жартівливі пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 17,
        "title": "Коломийки",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 18,
        "title": "Приспівки до танців",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 19,
        "title": "Народні романси. Жорстокі романси",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 19,
        "title": "Стрілецькі та повстанські пісні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 19,
        "title": "Псальми",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 19,
        "title": "Кічі",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 6,
        "sub_category_id": 19,
        "title": "Частушки",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 7,
        "sub_category_id": 20,
        "title": "Билини київського циклу",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 7,
        "sub_category_id": 21,
        "title": "Кобзарство. Думи",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 7,
        "sub_category_id": 22,
        "title": "Лірництво. Псальми і канти",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 8,
        "sub_category_id": 23,
        "title": "Самозвучні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 8,
        "sub_category_id": 23,
        "title": "Духові",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 8,
        "sub_category_id": 23,
        "title": "Струнні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 8,
        "sub_category_id": 23,
        "title": "Ударні",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 8,
        "sub_category_id": 24,
        "title": "Інструментальні капели. Троїста музика",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 8,
        "sub_category_id": 25,
        "title": "Питомі танці",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 8,
        "sub_category_id": 25,
        "title": "Напливові танці",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 9,
        "sub_category_id": 26,
        "title": "Голосіння",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
    {
        "main_category_id": 9,
        "sub_category_id": 27,
        "title": "Поминальні псальми",
        "description": LOREM,
        "media1": create_file_field("static/education_page_song_genres/1.png"),
        "media2": create_file_field("static/education_page_song_genres/2.png"),
        "media3": create_file_field("static/education_page_song_genres/3.png"),
    },
]
FAKE_EDUCATION = {
    "title": "Освітній розділ",
    "description": "Пріоритети сучасних – заповнення «білих плям» на фольклористичній карті України (східна Волинь, Наддніпрянщина, південна Чернігівщина, Сумщина, Берестейщина та інші території) та спеціалізовані регіональні програми з видання пісенних збірок за матеріалами певних етнографічних локусів.",
    "recommendations": f"<p>{LOREM}</p>",
    "recommended_sources": SOURCES,
}
FAKE_EXPED_CATEGORY = [
    {"title": "Розвідка"},
    {"title": "Статичний"},
    {"title": "Міждисциплінарна"},
    {"title": "Тематична"},
    {"title": "Відеозапис обряду"},
    {"title": "Цифровий запис"},
]
EXPEDITIONS_TITLES = [
    "Дослідження місцевих легенд",
    "Відкриття нових культурних звичаїв",
    "Експедиція до джунглів",
    "Реконструкція історичних подій",
    "Мистецтво та культура сучасного міста",
    "Вивчення фольклору та традицій",
    "Етнографічні дослідження",
    "Експедиція до невідомих місць",
    "Дослідження впливу природи на культуру",
    "Створення документального фільму про місцеві традиції",
]
PROJECT_TITLES = [
    "Дослідження українських обрядів",
    "Пошук унікальних народних мелодій",
    "Етнографічні експедиції в селах",
    "Фольклорні традиції сучасного міста",
    "Збереження української народної співанки",
    "Дослідження музичних звичаїв Гуцульщини",
    "Українські мелодії: минуле і сьогодення",
    "Культурні етнографічні експедиції",
    "Пошук народних танців та мелодій",
    "Спадщина українських колядок",
]


FAKE_PROJECTS = []
for i in range(10):
    title = random.choice(PROJECT_TITLES)
    short_description = f"Короткий опис проєкту '{title}'"
    preview_photo = create_file_field("static/projects/project1.png")
    project_date = datetime.datetime.now().date() - datetime.timedelta(days=i + 1)
    content = f"<p>Опис проєкту &#39;{title}&#39;.</p>"
    authors = random.sample(PEOPLE, random.randint(1, len(PEOPLE)))
    editors = random.sample(PEOPLE, random.randint(1, len(PEOPLE)))
    photographers = random.sample(PEOPLE, random.randint(1, len(PEOPLE)))
    city_id = random.randint(1, 39)

    fake_project = {
        "title": title,
        "short_description": short_description,
        "preview_photo": preview_photo,
        "project_date": project_date,
        "content": content,
        "authors": authors,
        "editors": editors,
        "photographers": photographers,
        "city_id": city_id,
    }
    FAKE_PROJECTS.append(fake_project)

FAKE_EXPEDITIONS = []
for i in range(10):
    title = random.choice(EXPEDITIONS_TITLES)
    short_description = f"Короткий опис експедиції '{title}'"
    map_photo = create_file_field("static/expeditions/Vector.png")
    preview_photo = create_file_field(f"static/expeditions/img{i + 1}.png")
    expedition_date = datetime.datetime.now().date() - datetime.timedelta(days=i + 1)
    content = f"<p>Опис експедиції &#39;{title}&#39;.</p>"
    category_id = random.randint(1, 6)
    city_id = random.randint(1, 39)
    authors = random.sample(PEOPLE, random.randint(1, len(PEOPLE)))
    editors = random.sample(PEOPLE, random.randint(1, len(PEOPLE)))
    photographers = random.sample(PEOPLE, random.randint(1, len(PEOPLE)))
    recording = random.sample(PEOPLE, random.randint(1, len(PEOPLE)))
    fake_expedition = {
        "title": title,
        "short_description": short_description,
        "map_photo": map_photo,
        "preview_photo": preview_photo,
        "expedition_date": expedition_date,
        "content": content,
        "category_id": category_id,
        "city_id": city_id,
        "authors": authors,
        "editors": editors,
        "photographers": photographers,
        "recording": recording,
    }
    FAKE_EXPEDITIONS.append(fake_expedition)

FAKE_PARTNERS = [
    {
        "photo": create_file_field("static/partners/1.svg"),
        "link": "https://knmau.com.ua/nauka/laboratoriya-etnomuzikologiyi/",
    },
    {
        "photo": create_file_field("static/partners/2.svg"),
        "link": "https://lnma.edu.ua/kafedry/kafedra-muzychnoji-folklorystyky-ta-pndlme/",
    },
    {
        "photo": create_file_field("static/partners/3.svg"),
        "link": "https://baza-trainee.tech/ua",
    },
]
FAKE_EXPEDITION_ABOUT = {
    "title": "Експедиції",
    "description": "Пріоритети сучасних – заповнення «білих плям» на фольклористичній карті України (східна Волинь, Наддніпрянщина, південна Чернігівщина, Сумщина, Берестейщина та інші території) та спеціалізовані регіональні програми з видання пісенних збірок за матеріалами певних етнографічних локусів.",
}
